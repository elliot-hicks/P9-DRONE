...
class AngledCustomController(FlightController):
    ...
    def get_thrusts(self, drone: Drone, parameters) -> Tuple[float, float]:
        ...
        try:
            theta_target = np.pi/2 - np.arctan(dy/(dx))
        ...
        try:
            theta_velocity = np.pi/2 - np.arctan(drone.velocity_y/drone.velocity_x)
        except ZeroDivisionError:
            theta_velocity = 0     
        if (drone.velocity_x<0):
            theta_velocity+=np.pi
            
        theta_rel = theta_target-theta_velocity  
        
        rotation_thrust = X*theta_rel
        
        if theta_velocity>np.pi:
            rotation_thrust *=-1
        if dx<0:
            rotation_thrust*=-1
        
        T1 = C + rotation_thrust - S*drone.pitch_velocity
        T2 = C - rotation_thrust + S*drone.pitch_velocity 
        
        if (abs(theta_rel)<0.2):
            T1+=Y/abs(theta_rel + 0.1)
            T2+= Y/abs(theta_rel + 0.1)
